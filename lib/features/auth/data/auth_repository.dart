...
class AuthRepository {
  ...
  Future<Response> _postRest(
    String route, {
    required Map<String, dynamic> data,
  }) async {
    final normalized = route.replaceFirst(RegExp(r'^/+'), '');
    final cleanRoute =
        normalized.replaceFirst(RegExp(r'^wp-json/+'), '');
    final dio = _dio();

    final primaryPath = '/wp-json/$cleanRoute';
    final primary = await dio.post(primaryPath, data: data);

    if (primary.statusCode != 404) return primary;

    final base = ApiConfig.wpBaseUrl.replaceFirst(RegExp(r'/+$'), '');
    final fallbackUrl = '$base/?rest_route=/$cleanRoute';

    await _log.log(
      'REST POST: مسیر اصلی 404 بود؛ تلاش fallback',
      level: 'WARNING',
    );

    return dio.post(fallbackUrl, data: data);
  }
  ...
  Future<void> login({
    ...
    try {
      ...
      await _log.log(
        'SERVER LOGIN: پاسخ HTTP ${res.statusCode?.toString() ?? 'unknown'} دریافت شد.',
      );

      await _persistLoginResponse(res);

      await _log.log(
        'SERVER LOGIN: پاسخ معتبر بود و اطلاعات نشست ذخیره شد.',
        level: 'SUCCESS',
      );
    } on ApiException catch (e) {
      await _log.log(
        'SERVER LOGIN FAILED: ${e.message}',
        level: 'ERROR',
      );
      rethrow;
  ...
  Future<void> loginWithMasterCode(String code) async {
    ...
    if (ApiConfig.demoLoginEnabled) {
      ...
      final token =
          'dev_session_${DateTime.now().millisecondsSinceEpoch}';
  ...
  Future<void> sendOtp(String mobile) async {
    ...
    try {
      ...
      if (res.statusCode != null && res.statusCode! >= 400) {
        throw ApiException(
          message: _err(data) ?? 'ارسال کد ناموفق',
        );
      }
  ...
  Future<OtpVerifyResult> verifyOtp({
    ...
    try {
      ...
      if (res.statusCode != null && res.statusCode! >= 400) {
        throw ApiException(
          message: _err(res.data) ?? 'کد نامعتبر است',
        );
      }
      ...
      throw ApiException(
        message: e.message ?? 'تأیید کد ناموفق بود',
      );
    }
  }
  ...
  Future<void> _persistLoginResponse(Response res) async {
    ...
    if (res.statusCode == 404) {
      throw ApiException(
        message:
            'مسیر ورود مدیریت پیدا نشد. REST API وردپرس یا پلاگین EzLens را بررسی کنید.',
      );
    }
    ...
    if (res.statusCode != 200 || data is! Map) {
      throw ApiException(
        message: _err(data) ?? 'ورود ناموفق بود',
      );
    }
    ...
    if (map['success'] == false) {
      throw ApiException(
        message: _err(map) ?? 'ورود ناموفق بود',
      );
    }
